package com.company.programmers.Lv1.신규아이디추천;

class Solution {

    public String solution(String new_id) {
        //1단계
        new_id = new_id.toLowerCase();
        //2단계
        String match = "[^a-z0-9-_.]";
        new_id = new_id.replaceAll(match,"");
        //3단계
        while(new_id.contains("..")) {
            new_id = new_id.replace("..",".");
        }
        //4단계
        char dot = '.';
        if(new_id.charAt(0)==dot) new_id = new_id.substring(1);
        else if (new_id.charAt(new_id.length()-1)==dot) {
            new_id = new_id.substring(0,new_id.length()-1);
        }
        //5단계
        if(new_id.isEmpty()) new_id = "a";
        //6단계
        if(new_id.length()>=16) {
            new_id = new_id.substring(0,15);
        }
        if (new_id.charAt(new_id.length()-1)==dot) {
            new_id = new_id.substring(0,new_id.length()-1);
        }
        //7단계
        if(new_id.length() <= 2) {
            String last = new_id.substring(new_id.length()-1);
            StringBuilder new_idBuilder = new StringBuilder(new_id);
            while(new_idBuilder.length() < 3) {
                new_idBuilder.append(last);
            }
            new_id = new_idBuilder.toString();
        }
        return new_id;
    }
}
